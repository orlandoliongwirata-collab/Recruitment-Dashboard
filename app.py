...
def load_data():
    ...
    # Bersihkan nama kolom (Kapital di awal, hapus spasi)
...
try:
    ...
    # --- PROSES MEMBERSIHKAN ANGKA ---
    # Fungsi ini menghapus Rp, %, dan koma agar jadi angka murni
    ...
    df_bulan['Target'] = df_bulan['Target'].apply(clean_number)
    df_bulan['Realisasi'] = df_bulan['Realisasi'].apply(clean_number)
    df_bulan['Nilai'] = df_bulan['Nilai'].apply(clean_number)
    ...
    # Grafik Batang
    fig = px.bar(df_kpi_filtered, 
                 x=df_bulan.columns[1], # Nama
                 y=['Target', 'Realisasi'], 
                 barmode='group', 
    ...
    fig.update_layout(legend_title_text='Keterangan')
    ...
    st.error(f"Terjadi kesalahan visualisasi: {e}")
