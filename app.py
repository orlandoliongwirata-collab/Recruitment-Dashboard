...
if not df.empty:
    ...
    if view == "🌍 Overview Team":
        ...
        for i, row in df_rank.iterrows():
            # Reset index manual untuk label medali (karena sudah di-sort)
        ...
        # (Bagian Detail PIC tetap sama seperti sebelumnya)
        ...
        if not df_pic.empty:
            ...
            with c1:
                ...
                st.image(df_pic['FOTO'].iloc[0] if pd.notna(df_pic['FOTO'].iloc[0]) else "https://via.placeholder.com/150", use_container_width=True)
            with c2:
                fig = px.bar(df_pic, x='KPI', y='ACH_VAL', text_auto='.1f', title=f"Achievement: {target}", color_continuous_scale='PuRd')
